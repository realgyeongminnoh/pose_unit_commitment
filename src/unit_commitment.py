...
def solve_uc_snapshot(
    ...
    # system
    load = input_uc.load[time_period]                               # load [t] [b]
    # operational constraint
    p_min = input_uc.p_min                             # p_min [i]
    p_max = input_uc.p_max                             # p_max [i]
    ramp_up = input_uc.ramp_up                         # ramp_up [i]
    ...
    # If the unit is on, ensure its generation meets the minimum power
    model.addConstrs(
    ...
    output_uc.u[time_period] = np.array(model.getAttr("X", u).select()).reshape(num_units,)
    output_uc.p[time_period] = np.array(model.getAttr("X", p).select()).reshape(num_units,)