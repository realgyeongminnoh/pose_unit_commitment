...
def solve_ed(
    ...
    output_ed.z[time_period] = np.array(model.getAttr("X", z).select()).reshape(num_buses,) if let_blackout else z
    ...
    output_ed.smp[time_period] = CONSTR_LOAD.Pi
    output_ed.cost_reserve_up[time_period] = CONSTR_RESERVEUP.Pi
    output_ed.cost_reserve_down[time_period] = CONSTR_RESERVEDOWN.Pi